...
def add_collateral(collateral: address, collateral_amount: uint256, lend_amount: uint256):
    ...
    if collateral == WETH:
        if collateral_amount > 0 and fee_data.service_fee > 0:
            send(fee_data.service_fee_collector, collateral_amount * fee_data.service_fee)
        if lend_amount > 0:
        ...
        if collateral_amount > 0:
            ...
            if fee_data.service_fee > 0:
                self._safe_transfer(collateral, fee_data.service_fee_collector, collateral_amount * fee_data.service_fee)
        if lend_amount > 0:
...
def borrow_more(collateral: address, collateral_amount: uint256, lend_amount: uint256, debt: uint256, withdraw_amount: uint256):
    ...
    controller: address = ControllerFactory(CONTROLLER_FACTORY).get_controller(collateral)
    if collateral == WETH:
        Controller(controller).borrow_more(lend_amount, withdraw_amount, value=lend_amount)
    else:
        self._safe_transfer_from(collateral, OWNER, self, collateral_amount)
        self._safe_approve(collateral, controller, lend_amount)
        Controller(controller).borrow_more(lend_amount, withdraw_amount)
...
@payable
def __default__():
    assert msg.sender == OWNER or msg.sender == CONTROLLER_FACTORY, "ETH receive"
